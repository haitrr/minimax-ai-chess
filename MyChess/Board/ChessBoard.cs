...
namespace MyChess
{
    public static class ChessBoard
    {
        ...
        public static void UnSelect()
        {
            ...
            string moves = convertAllMovesToDigitMoves();
            int boxSize = boardHeight / 8;
            //drawSquare(selecting / 8, selecting % 8,false,false);
            //drawPiece(selecting / 8, selecting % 8);
            for(int i=0;i<moves.Length;i+=4)
            {
                string move = moves.Substring(i, 4);
                //if(char.GetNumericValue(move[0])==selecting/8&& char.GetNumericValue(move[1])==selecting%8)
                //{
                //    drawSquare((int)char.GetNumericValue(move[2]), (int)char.GetNumericValue(move[3]), false, false);
                //    drawPiece((int)char.GetNumericValue(move[2]), (int)char.GetNumericValue(move[3]));
                //}
            }
            selecting = -1;
        ...
        {
            string move="";
            switch(ChessGame.difficulty)
            {
                case ChessGame.Difficulty.easy:
                    move = Search.interationDeepening(3,0,500, WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white);
                    break;
                case ChessGame.Difficulty.veryEasy:
                    move = Search.interationDeepening(2,0, 500, WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white);
                    break;
                case ChessGame.Difficulty.hard:
                    move = Search.interationDeepening(4,2, 1000, WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white);
                    break;
                case ChessGame.Difficulty.veryHard:
                    move = Search.interationDeepening(99,0, 1000, WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white);
                    break;
            }
            makeMove(move);
            ChessGame.heuristicValueTextBox.Text = (Evalue.evalue(WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK,white?false:true)).ToString();
        }
        ...
        //        Console.ReadLine();
        //        depth = Int32.Parse(newdefth);
        //        Perft.perftRoot(WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white, depth);
        //    }
        Perft.perftRoot(WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ, white, 0);
        }
